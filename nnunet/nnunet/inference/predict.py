...
def preprocess_save_to_queue(preprocess_fn, q, list_of_lists, output_files, segs_from_prev_stage, classes,
    ...
    for i, l in enumerate(list_of_lists):
        try:
            ...
            # nii = nib.Nifti1Image(np.transpose(d[0], axes=[2, 1, 0]), affine=None)
            # nib.save(nii, output_file + "_test.nii")
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    # planner = load_pickle(join(model, "plans.pkl"))
    # if torch.cuda.is_available():
    #     g = hl.build_graph(trainer.network, torch.rand((1, 1, *planner['plans_per_stage'][1]['patch_size'])).cuda(),
    #                         transforms=None)
    # else:
    #     g = hl.build_graph(trainer.network, torch.rand((1, 1, *planner['plans_per_stage'][1]['patch_size'])),
    #                       transforms=None)
    # g.save(join(model, "network_architecture.pdf"))
