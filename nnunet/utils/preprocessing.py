...
def add_gaussian_noise(image, mean=0, std=1):
    ...
    # noise_img = image + gaus_noise / 255.0
...
def add_uniform_noise(image, low=-10, high=10):
    ...
    #image = image.astype("int16")
    # noise_img = image + (uni_noise / 255.0)
...
def change_brightness(image, value):
    """
    Args:
        image : numpy array of image (0 to 1)
        value : brightness (0 to 255)
    Return :
        image : numpy array of image with brightness added (0 to 1)
    """
    #image = image.astype("int16")
